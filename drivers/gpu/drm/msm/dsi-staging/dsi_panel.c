...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->reset_config.reset_gpio)) {
		...
		if (lcd_reset_keep_high)
			...
			pr_debug("[NVT-ts] lcd-reset_gpio = 0\n");
		}
#else
		gpio_set_value(panel->reset_config.reset_gpio, 0);
#endif
	...
	u32 bl_lvl)
{
	...
	if (rc) {
		pr_debug("[%s] failed to change pwm config, rc=\n", panel->name,
			rc);
		goto error;
	}
	...
	if (!bl->pwm_enabled) {
		...
		if (rc) {
			pr_debug("[%s] failed to enable pwm, rc=\n", panel->name,
				rc);
			goto error;
		}
...
int dsi_panel_set_feature(struct dsi_panel *panel,enum dsi_cmd_set_type type)
{
       ...
       pr_debug("xinj:%s panel_initialized=%d type=%d\n",__func__,panel->panel_initialized,type);
	if (!panel->panel_initialized) {
		pr_debug("xinj: con't set cmds type=%d\n",type);
		return -EINVAL;
	}
	...
	mutex_lock(&panel->panel_lock);
       ...
       if (rc) {
               pr_debug("[%s] failed to send DSI_CMD_SET_FEATURE_ON/OFF cmds, rc=%d,type=%d\n",
                      panel->name, rc,type);
       }
       mutex_unlock(&panel->panel_lock);
       return rc;
